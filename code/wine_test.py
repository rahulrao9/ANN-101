...
# to run use - python3 wine_test.py --wine_main
...
try:
    ...
    print(
        "Required Modules NOT FOUND! try pip install {module_name}"
    )
    sys.exit()
...
def test_case():
    ...
    try:
        ...
        if ((X_train.shape[0] + X_test.shape[0]) == X.shape[0] and X.shape[0] == (
                y_train.shape[0] + y_test.shape[0]) and np.allclose(np.mean(X_train), 0, atol=1e-1) and np.allclose(
                np.mean(X_test), 0, atol=1e-1)):
    ...
    try:
        ...
        if (len(model1.get_params()['hidden_layer_sizes']) == 3 and len(
                model2.get_params()['hidden_layer_sizes']) == 3 and model1.get_params()['activation'] !=
                model2.get_params()['activation']):
